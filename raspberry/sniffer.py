...
import os
import datetime
from scapy.all import sniff, Dot11, Dot11Elt, RadioTap, conf # type: ignore
...
buffer = []
timestamp_state = {"last_ts": datetime.datetime.now()}
timestamp_lock = threading.Lock()

def get_smart_timestamp():
    with timestamp_lock:
        now = datetime.datetime.now()
        # Jeśli pakiety wpadają za szybko, sztucznie popchnij o 5ms do przodu
        if now < timestamp_state["last_ts"] + datetime.timedelta(milliseconds=5):
            now = timestamp_state["last_ts"] + datetime.timedelta(milliseconds=5)
        
        timestamp_state["last_ts"] = now
...
def insert_rows(connection, rows, label):
    ...
    for r in rows:
        ...
        if result and result[0]:
            ...
            last_ts = datetime.datetime.fromisoformat(last_ts_string)
            current_ts = datetime.datetime.fromisoformat(r["timestamp"])
            diff_ms = (current_ts - last_ts).total_seconds() * 1000
            if diff_ms < TIMESTAMP_STEP:
                continue  # Pomiń pakiet, jeśli różnica jest mniejsza od stałego kroku czasu.
...
def main():
    ...
    parser.add_argument("--iface", default="wlx045ea4eeffe9", help="monitor-mode interface")
    parser.add_argument("--channels", default="1-165", help="comma-separated channels (f.e. 1,6,11 or 1-13)")
